...
public class JoystickManager extends Command {
	...
	protected void execute() {
		...
		if (driveThrottle != 0) {
			...
			if (driveThrottle > 1) {
				// Robot.ledSystem.theaterChase(0, 255, 0, (int)((driveThrottle
				// - 1)/-1 * 40));
			} else {
				// Robot.ledSystem.theaterChase(255, 0, 0, (int)((driveThrottle
				// + 1) * 40));
			}
			...
			// Robot.ledSystem.rainbowCycle((int)((Math.abs(turnThrottle) -1)/-1
			// * 40));
		} else if (auxTwistAxis != 0) {
			Robot.driveSystem.stationaryTurn(auxTwistAxis * .25);
		} else {
			if (auxThrottle > 0 && auxButtonSecondary) {
